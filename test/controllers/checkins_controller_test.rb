...
class CheckinsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "requires authentication to access checkin page" do
    get checkin_path
    assert_redirected_to new_session_path
  end

  test "requires GM or admin role to access checkin page" do
    login_with_otp(@player.email)
    get checkin_path
    assert_redirected_to root_path
  end

  test "allows admin to access checkin page" do
    login_with_otp(@admin.email)
    get checkin_path
    assert_response :success
  end

  test "allows GM to access checkin page" do
  ...
  test "admin can check in a seat via token" do
    ...
    post checkin_path, params: { token: @seat.qr_token }
    @seat.reload
    assert @seat.checked_in?
    assert_redirected_to checkin_path
  end
  ...
  test "invalid token shows error" do
    login_with_otp(@admin.email)
    post checkin_path, params: { token: "invalid_token" }
    assert_redirected_to checkin_path
    follow_redirect!
  ...
  test "GM cannot check in seats for other GMs games" do
    ...
    login_with_otp(@gm.email)
    patch checkin_seat_path(other_seat)
