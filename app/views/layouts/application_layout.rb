...
class Views::Layouts::ApplicationLayout < Views::Base
    ...
    html do
      head do
        ...
        if Rails.env.production?
          script(defer: true, src: "https://api.pirsch.io/pa.js", id: "pianjs", data_code: "oKRec8iQLQgKJr21uIOfvES9jNpmrB3P")
        end
      end
  ...
  def render_flash
    if helpers.alert.present?
      p(
        class: "inline-block self-center bg-red-50 mb-5 px-3 py-2 rounded-md font-medium text-red-500",
        id: "notice"
      ) { helpers.alert }
    end
    ...
    if helpers.notice.present?
      p(
        class: "inline-block self-center bg-green-50 mb-5 px-3 py-2 rounded-md font-medium text-green-500",
        id: "notice"
      ) { helpers.notice }
    end
  end
