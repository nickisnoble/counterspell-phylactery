            ...
            @heroes.each do |hero|
              div class: "bg-white rounded-lg border border-gray-200 p-6 hover:shadow-md transition-shadow" do
                ...
                div class: "flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-100" do
                  ...
                  button_to "Destroy", hero, method: :delete, class: "text-sm px-3 py-1.5 bg-red-100 hover:bg-red-200 text-red-700 rounded-md font-medium cursor-pointer transition-colors", data: { turbo_confirm: "Are you sure?" }
                end
              end
            end
          end
