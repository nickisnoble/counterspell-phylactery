class Views::Traits::FormComponent < Views::Base
  ...
  def view_template
    form_with(model: @trait, class: "contents text-left") do |form|
      render_errors(form) if @trait.errors.any?
      ...
      div class: "my-5", data: { controller: "abilities-editor", abilities: @trait.abilities.to_json } do
        ...
        div class: "mt-2" do
          ...
          button type: "button", data: { action: "click->abilities-editor#addAbility" } do
            "+ Add Ability"
          end
        end
        ...
        template data: { abilities_editor_target: "template" } do
          div class: "ability-row" do
            input type: "text", placeholder: "Ability name", class: "ability-key", required: true
            input type: "text", placeholder: "Description", class: "ability-value", required: true
            button type: "button", data: { action: "click->abilities-editor#removeAbility" } do
              "Remove"
            end
          end
        end
  ...
  def render_errors(form)
    div id: "error_explanation", class: "bg-red-50 text-red-500 px-3 py-2 font-medium rounded-md mt-3" do
      h2 do
        plain pluralize(@trait.errors.count, "error")
        plain " prohibited this trait from being saved:"
      end
